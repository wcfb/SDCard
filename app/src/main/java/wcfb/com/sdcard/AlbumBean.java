...
public class AlbumBean {
     ...
     * 查询手机上的所有相册
     */
    public static void getAllAlbumFromLocalStorage(final Context context,
                                                   final AlbumListCallback completeCallback) {
        new MultiTask<Void,Void,ArrayList<AlbumBean>>(){
            ...
            protected ArrayList<AlbumBean> doInBackground(Void... params) {
                ...
                //首先查询一共有多少张
                Cursor countCursor = mContentResolver.query(mImageUri,
                        new String[]{"COUNT(*) "},
                ...
                //第一列
                int photoCount = countCursor.getInt(0);
                Log.i("wcfb","该手机的照片总量是:"+photoCount);
                countCursor.close();
                if(photoCount == 0)
                    return null;
                HashMap<String,AlbumBean>albumMap = new HashMap<>();
                int index = 0;
                //获取手机的相册列表
                while (index < photoCount){
                    //为了手机里有几万张照片OOM，500张为一组进行扫描相册列表
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if(albumMap.containsKey(parentFolder)){
                            //这个相册已经被记录过了
                ...
                Log.i("wcfb","所有图片扫描完毕,相册列表是"+albumList);
                return albumList;
            }
    ...
    public static void getAlbumPhotosFromLocalStorage(final Context context,
                final AlbumBean album, final AlbumPhotosCallback completeCallback) {
        ...
        new MultiTask<Void,Void,ArrayList<SelectPhotoAdapter.SelectPhotoEntity>>(){
            ...
            protected ArrayList<SelectPhotoAdapter.SelectPhotoEntity> doInBackground(Void... params) {
                ArrayList<SelectPhotoAdapter.SelectPhotoEntity> photoList = new ArrayList<>();
                Uri mImageUri = MediaStore.Images.Media.EXTERNAL_CONTENT_URI;
                ...
                if("Recently".equals(album.folderName))
                    mCursor = mContentResolver.query(mImageUri,
                        new String[]{MediaStore.Images.Media.DATA},
                        MediaStore.Images.Media.MIME_TYPE + "=? or "
                                + MediaStore.Images.Media.MIME_TYPE + "=?",
                        new String[]{"image/jpeg", "image/png"},
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    //经常会有相册相互包含的情况
                    if(!"Recently".equals(album.folderName) &&
                            !new File(path).getParentFile().getAbsolutePath().equals(album.albumFolder.getAbsolutePath()))continue;
