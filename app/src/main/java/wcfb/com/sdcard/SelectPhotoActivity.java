...
public class SelectPhotoActivity extends AppCompatActivity implements SelectPhotoAdapter.CallBackActivity,
    ...
    private void initView(){
        GridView gvPhotoList = findViewById(R.id.gv_photo);
        allPhotoAdapter = new SelectPhotoAdapter(this,
                new ArrayList<SelectPhotoAdapter.SelectPhotoEntity>());
        gvPhotoList.setAdapter(allPhotoAdapter);
        tv_done = findViewById(R.id.tv_done);
        tv_album_name = findViewById(R.id.tv_album_name);
        findViewById(R.id.rl_center).setOnClickListener(this);
        iv_showalbum = findViewById(R.id.iv_showalbum);
        //选择相册的布局
        rl_album = findViewById(R.id.rl_album);
        rl_album.setOnClickListener(this);
        lv_albumlist = findViewById(R.id.lv_albumlist);
        ...
        permissionHelper.checkPermission(this, new PermissionHelper.AskPermissionCallBack() {
            ...
            public void onSuccess() {
                //扫描手机上500张最近注册过的图片
                SelectPhotoAdapter.get500PhotoFromLocalStorage(SelectPhotoActivity.this,
                        new SelectPhotoAdapter.LookUpPhotosCallback() {
                    @Override
                ...
                //查找并设置手机上的所有相册
                AlbumBean.getAllAlbumFromLocalStorage(SelectPhotoActivity.this,
                        new AlbumBean.AlbumListCallback() {
                    @Override
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent dataIntent) {
        ...
        switch (requestCode) {
            case SelectPhotoAdapter.REQ_CAMARA: {
                //1000如果是相机发来的
                ...
                if (RESULT_OK == resultCode) {//系统默认值
                    ...
                    File file = new File(dir, cameraPhotoUrl);
                    //这个文件指针指向拍好的图片文件
                    final String imageFilePath = file.getAbsolutePath();
                    Log.i("Alex", "拍摄图片暂存到了" + imageFilePath + "  角度是" + ImageLoader.readPictureDegree(imageFilePath));
                    //把图片压缩成指定宽高并且保存到本地
                    ...
                    try {
                        ...
                        //返回值为要发送图片的url
                       albumUrl = BitmapUtils.insertImage(this,
                               cr, file, true);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    ...
                    if(selectedPhotoList == null)
                        selectedPhotoList = new ArrayList<>(1);
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.tv_done:
                if(selectedPhotoList == null)
                    selectedPhotoList = new ArrayList<>(9);
